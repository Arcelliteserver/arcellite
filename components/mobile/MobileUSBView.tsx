...
const MobileUSBView: React.FC<{
  ...
  const handleMount = async (deviceName: string, pwd?: string) => {
    if (!pwd) { setPasswordModal({ isOpen: true, device: deviceName, action: 'mount' }); setPassword(''); return; }
    ...
    try {
      ...
      const d = await r.json();
      if (!r.ok) {
        if (d.requiresAuth) { setMountError('Invalid password'); setMountingDevice(null); return; }
        throw new Error(d.error || 'Mount failed');
      }
      setPasswordModal(null); setPassword('');
      fetchStorage();
    } catch (e: any) { setMountError(e.message); }
    setMountingDevice(null);
  };

  /* unmount */
  const handleUnmount = async (deviceName: string, pwd?: string) => {
    if (!pwd) { setPasswordModal({ isOpen: true, device: deviceName, action: 'unmount' }); setPassword(''); return; }
    ...
    try {
      ...
      const d = await r.json();
      if (!r.ok) {
        if (d.requiresAuth) { setMountError('Invalid password'); setUnmountingDevice(null); return; }
  ...
  return (
    <div className="animate-in fade-in duration-200">
      {/* Header */}
      <div className="mb-5">
        <h1 className="text-[28px] font-extrabold text-gray-900 tracking-tight leading-none">USB Devices</h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          ...
          <p className="text-[14px] font-medium text-gray-400 mt-1">Connect a USB device to get started</p>
        </div>
      ) : (
        <div className="space-y-3">
          {devices.map((dev) => {
            ...
            const isPortable = dev.deviceType === 'portable';
