...
import { HardDrive, FolderPlus, CheckCircle, Mail, Lock, User, Cloud, ArrowRight, ArrowLeft, AlertCircle, Loader2, Check, X, Eye, EyeOff, Usb, Server, ArrowRightLeft, Monitor, RefreshCw, Database, FolderOpen, Image, Video, Music, Share2, FileText, Bell, History, Zap } from 'lucide-react';
        ...
        {showTransferImport && !transferComplete && (
          <div className="bg-white rounded-[2rem] sm:rounded-[2.5rem] border border-[#5D5FEF]/20 shadow-2xl shadow-[#5D5FEF]/10 p-6 sm:p-8 md:p-10 mb-6">
            {!transferImporting ? (
              ...
              /* Importing progress — detailed view */
              <div className="space-y-4 py-2">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="text-base font-black text-[#111111] mb-0.5">Importing Data...</h2>
                    <p className="text-gray-400 text-[10px] font-medium">Do not disconnect the USB drive</p>
                  </div>
                ...
                <div className="flex items-center gap-1 text-[9px] font-bold uppercase tracking-wider">
                  {[
                    { id: 'account', label: 'Account', phases: ['importing-account'] },
                    { id: 'database', label: 'Database', phases: ['importing-db'] },
                    { id: 'files', label: 'Files', phases: ['importing-files'] },
                  ].map((step, i) => {
                    const phase = transferStatus?.phase || '';
                    const isActive = step.phases.includes(phase);
                    const isDone = (() => {
                      const phaseOrder = ['importing-account', 'importing-db', 'importing-files', 'done'];
                ...
                <div className="bg-[#F5F5F7] rounded-xl p-3 space-y-2">
                  ...
                  {/* Current file name */}
                  {transferStatus?.currentFile && transferStatus.phase === 'importing-files' && (
                ...
                {transferStatus?.categories && transferStatus.categories.length > 0 && transferStatus.phase === 'importing-files' && (
                  <div className="space-y-1">
            ...
            <div className="space-y-5">
              <div className="text-center">
                ...
                <p className="text-gray-400 text-sm font-medium mt-1">
                  Your data has been fully restored. You can safely remove the USB drive.
                </p>
              </div>
                ...
                <div className="bg-[#F5F5F7] rounded-2xl p-4 space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    <div className="bg-white rounded-xl p-2 text-center">
                      <p className="text-sm font-black text-[#5D5FEF]">
                        {transferStatus?.filesCopied?.toLocaleString() || selectedTransfer.manifest.totalFiles.toLocaleString()}
                      </p>
                      <p className="text-[8px] font-bold text-gray-400 uppercase tracking-wider">Files</p>
                    </div>
                    <div className="bg-white rounded-xl p-2 text-center">
                      <p className="text-sm font-black text-[#5D5FEF]">
                        {transferStatus?.bytesWritten
                          ? transferStatus.bytesWritten > 1024 * 1024 * 1024
                            ? `${(transferStatus.bytesWritten / (1024 * 1024 * 1024)).toFixed(1)}GB`
                            : `${(transferStatus.bytesWritten / (1024 * 1024)).toFixed(0)}MB`
                          : '—'}
                      </p>
                      <p className="text-[8px] font-bold text-gray-400 uppercase tracking-wider">Data</p>
                    </div>
                    <div className="bg-white rounded-xl p-2 text-center">
                      <p className="text-sm font-black text-[#5D5FEF]">
                        {transferStatus?.dbRows
                          ? transferStatus.dbRows.reduce((s: number, r: any) => s + r.imported, 0).toLocaleString()
                          : '—'}
                      </p>
                      <p className="text-[8px] font-bold text-gray-400 uppercase tracking-wider">Records</p>
                    </div>
                  </div>
                  ...
                  {transferStatus?.categories && transferStatus.categories.filter((c: any) => c.totalFiles > 0).length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {transferStatus.categories.filter((c: any) => c.totalFiles > 0).map((cat: any) => (
                        <span key={cat.name} className="inline-flex items-center gap-1 px-2 py-0.5 bg-green-50 text-green-700 rounded-full text-[9px] font-bold">
