...
const ManageStorageView: React.FC = () => {
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex flex-row items-center justify-between mb-4 sm:mb-6 md:mb-8 gap-3 sm:gap-4">
        <div className="relative flex-1 min-w-0">
          ...
          <p className="text-[11px] sm:text-xs md:text-sm text-gray-400 font-medium mt-0.5 sm:mt-1 pl-3 sm:pl-4 md:pl-6">View usage, track categories, and request more space</p>
        </div>
      ...
      {loading ? (
        ...
        <>
          {/* ── Your Storage — full-width top card ── */}
          <div className="bg-gradient-to-br from-[#5D5FEF] to-[#4B4DD9] rounded-2xl sm:rounded-3xl p-5 sm:p-7 text-white shadow-xl shadow-[#5D5FEF]/15 mb-6 sm:mb-8">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-wider text-white/60">Your Storage</p>
                  <p className="text-[20px] sm:text-[24px] font-black leading-tight">
                    {storage ? storage.usedHuman : '—'} <span className="text-[14px] font-bold text-white/60">/ {storage ? storage.quotaHuman : '—'}</span>
                  </p>
                </div>
              </div>
              <div className="hidden sm:flex items-center gap-4 sm:gap-6">
                <div className="text-right">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-white/50">Used</p>
                  <p className="text-[16px] sm:text-[18px] font-black">{storage?.usedHuman || '—'}</p>
                </div>
                <div className="w-px h-8 bg-white/20" />
                <div className="text-right">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-white/50">Free</p>
                  <p className="text-[16px] sm:text-[18px] font-black">{storage?.freeHuman || '—'}</p>
                </div>
                <div className="w-px h-8 bg-white/20" />
                <div className="text-right">
                  <p className="text-[10px] font-bold uppercase tracking-wider text-white/50">Categories</p>
                  <p className="text-[16px] sm:text-[18px] font-black">{categories.length}</p>
                </div>
              </div>
            </div>

            {/* Progress bar */}
            <div className="h-3 w-full rounded-full bg-white/20 mb-3 overflow-hidden">
              <div
                className={`h-full rounded-full transition-all duration-700 ${pct >= 90 ? 'bg-red-400' : pct >= 70 ? 'bg-amber-400' : 'bg-white'}`}
          ...
          {/* ── Two columns: Need More Space + Request History (equal height) ── */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 mb-6 sm:mb-8">
            {/* ── Need More Space ── */}
            <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 p-5 sm:p-6 shadow-sm flex flex-col">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#5D5FEF] to-indigo-600 flex items-center justify-center shadow-sm">
                  <ArrowUpCircle className="w-4.5 h-4.5 text-white" />
                </div>
                <div>
                  <h2 className="text-[14px] sm:text-[15px] font-black text-gray-900">Need More Space?</h2>
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 p-5 sm:p-6 shadow-sm flex flex-col">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h2 className="text-[14px] sm:text-[15px] font-black text-gray-900">Request History</h2>
                  <p className="text-[11px] text-gray-400 font-medium">Your most recent storage request</p>
                </div>
              </div>
              ...
              {(() => {
                ...
                if (!latestRequest) {
                  return (
                    <div className="flex-1 flex flex-col items-center justify-center py-4 sm:py-6">
                      <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-2xl bg-[#5D5FEF]/5 flex items-center justify-center mb-3">
                        <Clock className="w-7 h-7 sm:w-8 sm:h-8 text-[#5D5FEF]/40" />
                      </div>
                      <p className="text-[12px] sm:text-[13px] text-gray-400 font-medium text-center">No requests yet</p>
                      <p className="text-[11px] text-gray-300 font-medium text-center mt-0.5">Your storage requests will appear here</p>
                    </div>
                  );
                }
                return (
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className={`flex items-center gap-2 p-3 mb-4 rounded-xl border ${
                      ...
                      <p className={`text-[12px] font-bold ${
                        latestRequest.status === 'approved' ? 'text-[#5D5FEF]' : latestRequest.status === 'denied' ? 'text-red-700' : 'text-amber-700'
                      }`}>
                        {latestRequest.status === 'approved'
                    ...
                    <div className="space-y-3 flex-1">
                      <div>
                        <label className="text-[11px] font-bold text-gray-600 uppercase tracking-wider mb-1 block">Amount Requested</label>
                        <div className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-[13px] font-bold text-gray-900 bg-gray-50">
                          +{latestRequest.requestedHuman}
                        </div>
                      </div>
                      {latestRequest.reason && (
                        <div>
                          <label className="text-[11px] font-bold text-gray-600 uppercase tracking-wider mb-1 block">Your Reason</label>
                          <div className="w-full px-3 py-2.5 rounded-xl border border-gray-200 text-[13px] text-gray-900 bg-gray-50 line-clamp-3">
                            {latestRequest.reason}
                          </div>
                        </div>
                      )}
                      {latestRequest.adminNote && (
                        <div>
                          <label className="text-[11px] font-bold text-gray-600 uppercase tracking-wider mb-1 block">Admin Note</label>
                          <div className="w-full px-3 py-2.5 rounded-xl border border-[#5D5FEF]/20 text-[13px] text-[#5D5FEF] font-medium bg-[#5D5FEF]/5">
                            "{latestRequest.adminNote}"
                          </div>
                        </div>
                      )}
                      <p className="text-[10px] text-gray-400">{relativeTime(latestRequest.createdAt)}</p>
                      {latestRequest.status === 'pending' && (
                        <div className="pt-1">
                          <button
                            onClick={() => handleCancelRequest(latestRequest.id)}
                            disabled={cancellingId === latestRequest.id}
                            className="w-full flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-[12px] font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 transition-all disabled:opacity-50"
                          >
            ...
            <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 p-5 sm:p-6 shadow-sm">
              <div className="flex items-center justify-between mb-5">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h2 className="text-[15px] sm:text-[16px] font-black text-gray-900">Storage Breakdown</h2>
